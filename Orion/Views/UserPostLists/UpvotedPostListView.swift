...
struct UpvotedPostListView: View {
...
extension UpvotedPostListView {
	...
	private func getPosts() {
		...
		Task(priority: .userInitiated) {
			do {
				let thing = try await service.getUpvotedPosts(username: username, after: paginator.after)
				let tempPosts = thing.data.children.map { thingPost in
					return PostViewModel(post: thingPost.data, listType: .user(username))
				}
				await self.serviceCompleted(morePosts: tempPosts, after: thing.data.after, error: nil)
			} catch {
				await self.serviceCompleted(morePosts: nil, after: nil, error: error)
			}
		}
	}
